...
@dsl.pipeline(name="mnist_nn")
def mnist_nn():
    data_preprocessing_task = data_preprocessing(dataset_path="/mnt/datasets/MNIST")
    ...
    # data_preprocessing_task = data_preprocessing_task.set_memory_request("858752Ki")
    # model_training_task = model_training_task.set_memory_request("858752Ki")
    # model_evaluation_task = model_evaluation_task.set_memory_request("171750Ki")


client = Client(host="http://kfp.local")
run = client.create_run_from_pipeline_func(pipeline_func=mnist_nn, enable_caching=False)
